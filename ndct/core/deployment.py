...
class Deployment:
	...
	def get_deployments_from_file():
		...
		'''
		if os.path.isfile('data/deployments.encrypted'):
			deployments_temp_file = Crypt.get_encrypted_file_contents('data/deployments.encrypted')
	...
	def save_deployments_to_file():
		...
		Crypt.create_encrypted_file('data/deployments', deployments_to_save)
